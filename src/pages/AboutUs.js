import React from "react"
import Photo1 from '../layout/photos/vasiliki_pantzia.jpg'
import Photo2 from '../layout/photos/ioanna_kanta.jpg'
import Photo3 from '../layout/photos/giorgos_tsilis.jpg'
import Photo4 from '../layout/photos/kostas_vournas.jpg'
import Photo5 from '../layout/photos/argiris_nikolaou.jpg'
import Photo6 from '../layout/photos/xristos_fourtounis.jpg'
import Photo7 from '../layout/photos/giannis_pantzias.jpg'

function AboutUs() {
    return (
        <div >
           
        <div className="container well-xs well-sm-inset-1">
            <div className="row flow-offset-1 txtalign">
                <div className="col-md-6 col-sm-6 txtalign ppading slideanim">
                    <h2 style={{color:"#FA2742"}}>Σχετικά με εμάς</h2>
                    
                    <h5 style={{color:"#FA2742"}}>ΕΣΤΙΑ City Services</h5>
                    <p >
                        Με εμπειρία πάνω από 30 χρόνια η ΕΣΤΙΑ CITY SERVICES πρωτοπορεί με τις υπηρεσίες της και τον επαγγελματισμό της. Διαθέτει ιδιόκτητες εγκαταστάσεις στον Βύρωνα και απασχολεί το πλέον έμπειρο προσωπικό που είναι καθημερινά δίπλα στους πελάτες για να εξυπηρετεί και να δίνει λύσεις σε κάθε τους πρόβλημα.
                    </p>
                    <p>
                        Η ΕΣΤΙΑ παρέχει μια σειρά από υπηρεσίες που καλύπτουν κάθε ανάγκη της σύγχρονης κατοικίας και αδιαμφισβήτητα βοηθά τον κάθε διαχειριστή να διατηρήσει τη σωστή και εύρυθμη λειτουργία του κτιρίου. Ωστόσο, οι υπηρεσίες μας δεν αφορούν μόνο σε πολυκατοικίες αλλά σε όλα τα είδη κτιρίων ή οικιστικών μονάδων όπως μέγαρα, κτίρια γραφείων, συγκροτήματα εξοχικών κατοικιών, εμπορικά κέντρα κλπ.
                    </p>
                    <p>
                        Στην πολυετή πορεία μας δε σταματήσαμε ποτέ να εκσυγχρονίζουμε το μηχανογραφικό μας σύστημα και να επενδύουμε σε νέες τεχνολογίες προκειμένου η παροχή των υπηρεσιών μας να βρίσκεται πάντοτε στο υψηλότερο δυνατό επίπεδο.
                    </p> <p>
                        <ul className="marked-list">
                            <li>Πρωτοπορήσαμε δημιουργώντας τους λογαριασμούς κοινοχρήστων.</li>
                            <li>Πετύχαμε την γρήγορη και εύκολη ενημέρωση των πελατών μας μέσω του ηλεκτρονικού μας συστήματος  Εstia-on.</li>
                            <li>Δώσαμε λύση στην ταλαιπωρία και το πρόβλημα ρευστότητας των πελατών μας, δίνοντας τους τη δυνατότητα εξόφλησης μέσω του site μας.</li>
                        </ul>
                    </p>
                    <br />
                    <p>
                        Είμαστε δίπλα σας για να σας εξυπηρετήσουμε στη διαχείριση του κτιρίου, στην έκδοση κοινοχρήστων, στην   καθαριότητα του χώρου σας, στην  απολύμανση, στην προμήθεια πετρελαίου,  τη συντήρηση του καυστήρα, τον καθαρισμό των δεξαμενών πετρελαίου, τον καθαρισμό αποχετεύσεων και την πυρόσβεση.
                    </p>
                    <p>
                        Βρισκόμαστε δίπλα σας  και σας παρέχουμε τις υπηρεσίες μας με αξιοπιστία, ποιότητα & οικονομία.
                    </p>
                    <p>
                        Θα χαρούμε να σας υποδεχτούμε στα γραφεία μας.
                    </p>
                </div>
                <div className="col-md-6 col-sm-6 slideanim">
                    <h2 style={{color:"#FA2742"}}>Τα πλεονεκτήματα μας</h2>
                    <p> <br /> </p>
                    <ol className="index-list avantage">
                        <li>
                            <h5 style={{color:"#FA2742"}}>Εξελισσομενο Μηχανογραφικο Συστημα </h5>
                            <p>
                                Διαθέτουμε το πλέον εκσυγχρονισμένο μηχανογραφικό σύστημα, που εξελίσσεται συνεχώς, με σκοπό να καλύπτει κάθε παλιά αλλά και νέας σας ανάγκη.  Βασικό μας στόχο αποτελούν η εύκολη ενημέρωση των πελατών μας, αλλά και η άμεση εξυπηρέτηση τους.
                            </p>
                        </li>
                        <li>
                            <h5 style={{color:"#FA2742"}}>Δυνατοτητα Ηλεκτρονικης Εξυπηρετησης </h5><p>
                                Η χρήση του διαδικτύου έχει επικρατήσει στην καθημερινότητα μας αλλά και στις υπηρεσίες. Όχι, άδικα αφού πλέον μπορείτε εύκολα και άμεσα να ενημερώνεστε, να προγραμματίζεστε αλλά και να πληρώνετε εύκολα και γρήγορα, χρησιμοποιώντας το site μας.
                                <br />Η Ομάδα μας αποτελείται από αξιόλογους ανθρώπους, έμπειρους στο αντικείμενο τους και συνεχώς εκπαιδευόμενους. Για την καλύτερη εξυπηρέτηση σας στις σύγχρονες ανάγκες η συνεχής κατάρτιση του προσωπικού μας θεωρείται απαραίτητη.
                            </p>
                        </li>
                        <li>
                            <h5 style={{color:"#FA2742"}}>Αξιοπιστοι Συνεργατες</h5><p>
                                Μία προσεχτικά επιλεγμένη ομάδα εξειδικευμένων τεχνιτών - συνεργατών, βρίσκεται δίπλα σας να σας εξυπηρετήσει σε κάθε τεχνικό πρόβλημα που αφορά στην πολυκατοικία σας ή στον επαγγελματικό σας χώρο.
                            </p>
                        </li>
                        <li>
                            <h5 style={{color:"#FA2742"}}>Ιδιοκτητες Εγκαταστασεις </h5><p>
                                Για να γνωριστούμε και από κοντά μπορείτε να μας επισκεφτείτε στις ιδιόκτητες εγκαταστάσεις μας στον Βύρωνα, Ν. Αρώνη 34. Βρισκόμαστε δίπλα σας  και σας παρέχουμε τις υπηρεσίες μας με αξιοπιστία, ποιότητα & οικονομία. Θα χαρούμε να σας υποδεχτούμε στα γραφεία μας για να γνωριστούμε από κοντά.
                            </p>
                        </li>
                    </ol>
                </div>
                
            </div>
        </div>
    


        <div className="container well-sm ">
            <h2 style={{color:"#FA2742"}}>Η ομάδα μας</h2>
            <div className="row flow-offset-1 text-xs-center slideanim">
                <div className="col-md-3 col-xs-6">
                    <img src={Photo1} alt="Vassiliki Pantzia"/>
                    <div className="box-skin-1 " >
                        <h5>ΠΑΝΤΖΙΑ Β. ΒΑΣΙΛΙΚΗ </h5>
                        <p>Manager & Σύμβουλος Διαχειρίσεων</p>
                        <p>Οικονομολόγος, Απόφοιτος Εθνικού & Καποδιστριακού Πανεπιστημίου Αθηνών </p>
                        <p>Msc Finanial Economics, Erasmus University of Rotterdam</p>
                    </div>
                </div>
                
                <div className="col-md-3 col-xs-6">
                    <img src={Photo2} alt="Ioanna Kanta"/>
                    <div className="box-skin-1 ">
                        <h5>ΚΑΝΤΑ Ο. ΙΩΑΝΝΑ</h5>
                        <p>Σύμβουλος Διαχειρίσεων & Τεχνική Υποστήριξη</p>
                        <p>Φιλόλογος,  Απόφοιτος Εθνικού & Καποδιστριακού Πανεπιστημίου Αθηνών</p>
                    </div>
                </div>
                <div className="col-md-3 col-xs-6">
                    <img src={Photo3} alt="Giorgos Tsilis"/>
                    <div className="box-skin-1" >
                        <h5>ΤΣΙΛΗΣ ΓΙΩΡΓΟΣ</h5>
                        <p>Σύμβουλος Διαχειρίσεων & Υπεύθυνος Επιστήμονας Απολυμάνσεων</p>
                        <p>Τεχνολόγος Γεωπόνος</p>
                    </div>
                </div>
                <div className="col-md-3 col-xs-6">
                    <img src={Photo4} alt="Kostas Vournas"/>
                    <div className="box-skin-1">
                        <h5>ΒΟΥΡΝΑΣ ΚΩΣΤΑΣ</h5>
                        <p>Επόπτης Κτιρίων  & Διανομέας - Εισπράκτορας</p>
                    </div>
                </div>
            </div>
            <div className="row flow-offset-1 text-xs-center slideanim">
                <div className="col-md-3 col-xs-6">
                    <img src={Photo5} alt="Argiris Nikolaou"/>
                    <div className="box-skin-1 ">
                        <h5>ΝΙΚΟΛΑΟΥ ΑΡΓΥΡΗΣ</h5>
                        <p>Επόπτης Κτιρίων  & Διανομέας – Εισπράκτορας</p>
                    </div>
                </div>
                <div className="col-md-3 col-xs-6">
                    <img src={Photo6} alt="Xristos Fourtounis"/>
                    <div className="box-skin-1">
                        <h5>ΦΟΥΡΤΟΥΝΗΣ ΧΡΗΣΤΟΣ</h5>
                        <p>Διανομέας – Εισπράκτορας & Απολυμαντής</p>
                    </div>
                </div>
                <div className="col-md-3 col-xs-6">
                    <img src={Photo7} alt="Giannis Pantzias"/>
                    <div className="box-skin-1 ">
                        <h5>ΠΑΝΤΖΙΑΣ Β. ΓΙΑΝΝΗΣ</h5>
                        <p>Υπεύθυνος Καθαριοτήτων</p>
                        <p>Απόφοιτος Τμήματος Διοίκησης Επιχειρήσεων </p>
                    </div>
                </div>
            </div>
        </div>
    
        </div>
    )
}

export default AboutUs